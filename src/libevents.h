...
private:
	EventManager(){}
	std::unordered_map < const char*, std::list < std::function<void(parameters)> >> _events;
...
public:
	...
	void trigger(const char* event_name, parameters params)
	{
	...
	void addEvent(const char* event_name, std::function<void(parameters)> callback)
	{
...
inline void register_event(const char* event_name, void(*callback)())
{
	std::function<void(parameters)> wrapper = [callback](parameters param) { callback(); };
	EventManager::I().addEvent(event_name, wrapper);
}

template <typename ... types>
inline void trigger_event(const char* event, types... _parameters)
{
...
template <>
inline void trigger_event(const char* event)
{
